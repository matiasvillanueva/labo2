...
FLAG_SCHEMA_VERSION = "cheap_v2"

FLAG_COLUMNS = [
    "llm_renovated",
    "llm_needs_work",
    "llm_major_repairs",
    "llm_mobile_manufactured",
    "llm_low_end_basic",
    "llm_affordable_language",
    "llm_luxury",
    "llm_waterfront_view",
    "llm_pool_spa",
    "llm_gated_security",
...
def flag_cache_tag(cache_tag: str = "") -> str:
    if cache_tag:
        return f"{cache_tag}_{FLAG_SCHEMA_VERSION}"
...
def flag_path(
    ...
    return CACHE_DIR / f"{split}_flags_{cache_suffix(provider, model, flag_cache_tag(cache_tag))}.csv"
...
def flag_chunk_dir(
    ...
    return CACHE_DIR / "flag_chunks" / f"{split}_{cache_suffix(provider, model, flag_cache_tag(cache_tag))}"
...
def _flag_prompt(items: list[dict]) -> str:
    return (
        ...
        "Return ONLY compact JSON: {\"rows\":[[zpid,ren,work,major_repairs,mobile,"
        "basic,afford,lux,water,pool,gated,new,distress,furn,hoa,park,outdoor,walk,"
        "quiet,cond,lux_score,loc_score,cheap_score],...]}\n"
        "Meanings: ren=renovated/updated; work=TLC/fixer/as-is; "
        "major_repairs=major repairs/renovation needed; mobile=mobile/manufactured/trailer home; "
        "basic=low-end/basic/no-frills/small housing; afford=affordable/starter/low-price language; "
        "lux=luxury/high-end; water=water/ocean/lake/canal view/front; pool=pool/spa; "
        "gated=gated/security; new=new construction; "
        "distress=foreclosure/short sale/investor/cash only; "
        ...
        "loc_score=location/view amenity score; cheap_score=overall cheap/low-value signal.\n"
...
def load_flag_features(
    ...
    out = flags.reindex(df["zpid"].values).reindex(columns=FLAG_COLUMNS, fill_value=0.0).fillna(0.0)
    out.index = df.index
